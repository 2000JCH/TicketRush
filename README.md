...
🔗 시연 영상: *(링크 추가 예정)* · 📄 포트폴리오: [TicketRush 부하/장애테스트_요약](https://app.notion.com/p/3da8d16f89fd81bd894ec64b76704348)
...
| 지표 | 목표 | 실측 | 판정 |
|---|---|---|---|
| 동시 300명 좌석 홀드 P95 | < 2,000ms | **883ms** | ✅ |
| 오버셀 | 0건 | **0건** (전 측정 세션 누적) | ✅ |
| 에러율 (경합 제외) | < 1% | 0% (N=300 기준) | ✅ |
| 한계 동시 사용자 | (참고, SLO 아님) | 450\~500명 (그 이상은 Redis 명령 타임아웃으로 절벽 — 원인까지 진단) | 참고 |
...
| 시나리오 | 결과 |
|---|---|
| **A-1 — Redis 73초 완전 다운 → 복구** | 오버셀 **0** · `seat_status` 재구성 완료까지 **18초** · 재구성 중 요청은 `503`으로 즉시 실패(부분 상태 미노출) |
