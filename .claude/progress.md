...
## 2026-09-14 완료 — 이력서·포트폴리오·README 숫자 불일치 전수 정리 + v2를 공식값으로 확정

이력서(Notion)와 포트폴리오(HTML/PDF, Notion) 숫자를 대조하는 과정에서 여러 문서(`test-results.md`/`README.md`/포트폴리오 HTML·PDF/이력서)에 **v1(09-06)과 v2(09-10) 숫자가 뒤섞여 들어가 있던 것**을 전부 찾아 정리했다.

**발견한 문제**:
1. 이력서 두 섹션이 각각 883ms(v2)/1,512ms(v1)를 따로 쓰고 있었음 — 어느 게 "진짜"인지 불명확했음.
2. 카오스 A-1(Redis) 장애시간에 A-2(Kafka)의 97초가 잘못 섞여 "61~97초"로 표기된 곳이 여러 문서(이력서·README·포트폴리오)에 있었음 — A-1의 실제 값은 73초(v2)/61초(v1/로컬).
3. 포트폴리오(HTML+PDF) 한계 테스트 표(N=100~500 P95/P99)가 `test-results.md` 어디에도 없는 값이었음 — Grafana 스크린샷 확인 결과, 실제로는 `uri=All`(전체 API 뒤섞인 값, 표보다 나쁘게 보임)과 `uri=좌석홀드 필터`(표와 정확히 일치) 패널을 혼동한 것으로 판명. `aws-remeasure-v2/load-test/`(uri 필터 적용) 스크린샷으로 5-2-v2 표 6개 구간(N=100/250/300/350/450/500) 전부를 직접 대조 검증 완료.
4. README.md에 오타(P95 883→875) 및 A-1 97초 오류가 남아있었음.

**결정(사용자 확인 완료) — v2(2026-09-10)를 외부 문서 공식값으로 채택**: v1·v2는 같은 스펙·코드·방법론이지만 EC2/RDS를 재생성해 실행한 별개 인스턴스라 결과가 갈렸다(883 vs 1,512ms 등) — 방법론 오류가 아니라 클라우드 인스턴스 재생성에 따른 정상적 변동폭. v2를 고른 이유는 (1) 더 최근이고 (2) uri 필터 스크린샷으로 사진 대조 검증까지 마쳤기 때문. v1 수치는 삭제하지 않고 최초 측정 기록으로 보존.

**반영 완료**:
- `test-results.md`: 맨 위에 "공식 채택값" 요약표 신설(다음에 또 헷갈리지 않도록), 5-1/5-2-v2/5-4에 공식값 포인터·근거 추가, 끊어진 스크린샷 경로(`aws-remeasure/`→`aws-remeasure-v1-archive/`) 수정.
- `aws-spec.md`: D-1/D-2/E 섹션에 v1·v2 값을 나란히 표기하고 공식값(v2) 명시.
- `README.md`: P95 875→883ms 오타 수정, A-1 97초→73초 수정.
- `docs/portfolio/정찬혁_TicketRush_포트폴리오.html`: A-1 장애시간 97→73초(본문+표 2곳), 한계 테스트 표 전체를 5-2-v2 값으로 교체, "핵심 결과" 표에 빠져있던 동시 300명 P95(883ms) 행 추가.
- 이력서(Notion), 포트폴리오(Notion 페이지)는 사용자가 직접 883ms/73초로 수정 완료(대화 중 확인).

**아직 남은 것(사용자 처리 필요)**:
- `.claude/임시참조폴더/TicketRush_부하장애테스트_요약.pdf`는 위 HTML 수정 이전 버전이라 **여전히 구값**(75초 아님 97초, 옛 한계테스트 표)이 들어있음 — Notion 포트폴리오 페이지에서 다시 내보내거나 수정된 HTML을 재출력해야 함.
- Notion 이력서/포트폴리오가 이번에 로컬 문서와 최종적으로 합의된 값(883ms, 73초, 97초, ~450~500명)과 전부 일치하는지 최종 1회 재확인 권장.

**같은 날 이어서 — "한계 테스트" 그래프-표 불일치 발견 및 정정**: 사용자가 "왜 N=300이 N=350보다 느리냐"고 질문한 걸 계기로 스크린샷을 다시 대조하다가, `capacity-limit`(uri=All, 전체 API 뒤섞임) 스크린샷과 `load-test`(uri=좌석홀드 필터) 스크린샷이 **같은 시각·같은 실행의 두 가지 다른 필터 뷰**라는 걸 확인했다. 포트폴리오의 "한계 테스트" 섹션 이미지가 표(좌석홀드 기준)와 안 맞는 `capacity-limit`(uri=All) 걸 쓰고 있었던 것도 확인.

- `test-plan.md` 1-2번을 근거로 **"한계 테스트"가 원래부터 "좌석 홀드→결제" 구간만 재도록 사전 정의돼 있었다**는 것도 확인(대기열 폴링 체감 대기는 별도 이탈률 테스트가 담당) — 이 스코프를 `test-results.md`와 포트폴리오에 명시 문구로 추가.
- **반영 완료**: `test-results.md`(공식 채택값 표 아래에 스코프 설명 추가) / 로컬 HTML(`docs/portfolio/정찬혁_TicketRush_포트폴리오.html` — 이미지를 `load-test/API응답시간.png`로 교체, 캡션을 "100~450 오르내림(300→350 반전 포함), 500에서만 구조적 붕괴"로 수정, 섹션 4 도입부에 스코프 문장 추가).
- **Notion 포트폴리오는 텍스트만 반영, 이미지 교체는 실패**: 표 6개 값·도입부 스코프 문장·캡션 문구는 Notion에도 반영했지만, **그래프 이미지 자체는 여전히 옛날 것(uri=All)** — Notion이 이미지 URL을 매 조회마다 새로 서명해서 내려주는 구조라 텍스트 검색-치환으로 이미지를 못 바꿈. 사용자가 Notion에서 직접 이미지를 교체해야 함(아래 "내일 할 일" 참고).
- **사용자 지시**: "Notion은 내가 수정해달라고 할 때까지 수정하지 말아줘" — 이후 Notion 이력서·포트폴리오는 명시적 요청 없이는 손대지 않음(읽기만 가능).

## 내일 할 일 (2026-09-15)

**1) Notion 포트폴리오 "4. 한계 테스트" 섹션 수정 — 완료(2026-09-14)**
- 응답시간 이미지 → `load-test/API응답시간.png`로 교체, 캡션 부착 완료.
- 에러율 이미지 → `load-test/에러율.png`로 교체 완료(기존 `capacity-limit/`(uri=All)에서 교체).
- "N=300→350 반전" 문단 추가 완료(원인 진단 콜아웃과 GitHub 링크 사이).
- 문구 다듬기: "구조적 붕괴" → "절벽"로 통일(공식 문서 `test-results.md`가 쓰는 용어와 맞춤, 과장된 표현 수정). Notion + 로컬 HTML(`docs/portfolio/정찬혁_TicketRush_포트폴리오.html`) 둘 다 반영. 로컬 HTML의 에러율 이미지(`images/capacity-error.png`)도 이때 같이 `load-test/에러율.png`로 교체(이전엔 Notion만 바뀌고 로컬은 누락돼 있었음).
- 전체 페이지(TicketRush+ClassQ) 표/이미지/텍스트 정합성 재확인 완료 — 추가 불일치 없음.

**2) 포트폴리오 PDF 재생성 — 안 함(사용자 결정, 2026-09-15)**. 더 이상 할 일 아님.

**3) 최종 일치 확인 — 완료(2026-09-15)**. Notion 이력서 + Notion 포트폴리오 + README + 로컬 HTML 4곳 대조:
- P95 883ms: 4곳 전부 일치.
- A-1(Redis) 73초 / A-2(Kafka) 97초: 포트폴리오·README·로컬HTML은 단일값(73초/97초)으로 일치. **Notion 이력서만 "61~73초"/"91~97초" 범위 표기**(로컬+AWS 값을 둘 다 보여줌) — 대조 결과 두 범위 다 공식값(73초/97초)을 포함하고 있어 **사실관계 오류 아님**, 형식만 다름. 사용자 확인 후 **그대로 두기로 결정**.
- 한계 동시 사용자: 이력서에만 구체적 인원수(450~500명) 없이 정성적 설명만 있음 — 마찬가지로 그대로 둠.
- (참고) `progress.md` 2026-09-14 기록엔 "이력서 883ms/73초로 수정 완료"라고 돼 있었는데, 재확인 결과 P95만 883ms로 고쳐져 있고 73초/97초는 범위 표기가 원래부터 유지되고 있었음 — 기록이 부정확했던 부분, 이번에 바로잡음.

**4) 사람인 이력서 "담당업무 확정본" 갱신 — 완료(2026-09-15)**. P95 1,512→883ms, Redis 재구성 5→18초(목표 30초 이내 문구도 추가, P95 줄과 형식 통일) 반영. 최종본은 아래 "이력서 담당업무 확정본" 블록에 갱신.

**5) 아직 안 건드린 것(참고만, 급하지 않음)** — AWS v1의 `a1-redis-down`/`a2-kafka-down` 스크린샷은 이번 정리에서 대조 안 함(로컬+v2로 이미 충분히 검증됐다고 판단). 필요하면 나중에 요청.
...
## 2026-09-11 진행 — 이력서 TicketRush 섹션 사실검증·수정 완료

이력서(Notion, "정찬혁-이력서 (1)")의 TicketRush 프로젝트 섹션(핵심 성과 + 세부 5항목: 대기열·좌석 동시성/분산락 벤치마크/카오스 테스트/Outbox 패턴/한계 테스트)을 `test-results.md`·실제 코드와 한 줄씩 대조. 수정 반영: ① P95 875→883ms(오타, 최신 v2 값) ② "완전 중단 카오스 테스트 오버셀..." 화살표 추가(인과관계 명확화) ③ "입장 메터링"→"입장 속도 제어" ④ 분산락 "문제" 문장 쉬운 말로 재작성 ⑤ 한계 테스트 "결과"에 "DB 병목 AWS에서 완전히 해소 확인" 긍정적 사실 추가. 지어낸 근거·환경(AWS/로컬) 혼용 없음을 전부 확인 — 유일하게 약한 근거는 분산락 벤치마크("성능 차이는 실측, 확장성 결론은 추론"임을 인지하고 유지, 뺄 필요 없음).

**TicketRush 포트폴리오(Notion) 검토는 2026-09-12로 미룸**(사용자 요청, 오늘은 다른 일정).

## 내일 할 일 (2026-09-12)

**1) TicketRush 포트폴리오(Notion 페이지) 다듬기** — 이력서와 같은 방식으로 사실관계 확인, 문장 다듬기, 추가/삭제 판단.

**2) 면접 대비 — 전체 내용 학습 + 예상 질문 준비**
- TicketRush 이력서·포트폴리오 + **ClassQ 포트폴리오까지 포함해서** 전체 내용 공부(숫자·근거·트레이드오프 다시 설명할 수 있게)
- 자소서 포함, 면접에서 일반적으로 나올 만한 예상 질문 리스트 뽑기
- 각 질문에 대한 답변을 생각하며 공부·암기
...
### 이력서 담당업무 확정본 (사람인, 2026-09-09 세션에서 확정 → 2026-09-15 v2 공식값으로 갱신)
...
[성과]
- 동시 사용자 300명 기준 좌석 홀드 P95 883ms (목표 2초 이내)
- Redis 약 1분 완전 중단 → 복구 후 좌석 상태 재구성 약 18초(목표 30초 이내), 복구 구간에서도 오버셀 0건
...
- **2026-09-06 (저녁)**: **로컬 시연 준비 — 프론트 PortOne 결제창 연동 + "A그룹" 회원 화면 개선.** 사용자가 로컬에서 시연 흐름을 돌려보며 부족한 부분을 정리해준 걸 반영했다.
  ...
  - **미결로 남은 것**: `payment.processing-timeout-millis`(placeholder) 확정 필요 — 카운트다운/결제 타임아웃의 실제 값.

- **2026-09-06 (밤)**: **"B그룹" 관리자 콘솔 완료.** 사용자가 시연 흐름을 점검하며 정리한 요청(관리자/주최자 화면 부재, 계정 정지, 매출·좌석 현황) 중 관리자 콘솔 부분만 구현했다. **C그룹(주최자가 브라우저에서 콘서트 생성하는 UI + 시간·좌석배치 입력)은 제외**(사용자 확인) — 콘서트는 여전히 API로 직접 등록.
  - **B0 프론트 role**: 로그인 응답에 role을 넣는 대신, 로그인/세션 복구 후 `AuthContext`가 `GET /accounts/me`(A그룹 신규)로 role을 읽어 보관. `ProtectedRoute`에 `adminOnly` 프롭 추가, 관리자 네비 메뉴는 `role === "ADMIN"`일 때만 노출. (백엔드 `LoginResponse`/`AuthService`는 안 건드림 — 위험 축소.) **2026-09-07 추가**: ADMIN으로 로그인하면 헤더에서 "내 예약"을 숨긴다(`Layout.tsx` — ADMIN은 관리자 메뉴 3개 + 내 정보만, BUYER/ORGANIZER는 내 예약 + 내 정보). 관리자는 개인 예약 개념이 없어서.
  - **B1 주최자 승인 화면**(`/admin/approvals`): 대기 목록 + 승인 / 거절(거절 = `DELETE /admin/accounts/{id}`, PENDING ORGANIZER 행 삭제, 신규).
  - **B2 회원 목록**(`/admin/members`): 신규 `GET /admin/accounts` — `role`/`status`/`email`(부분일치) 필터 + 페이지네이션(`AccountRepository.search`의 nullable 파라미터 JPQL + `countQuery`, 최근 가입 순). 공통 `PagedResponse<T>` DTO 신설(`global/dto`). 실측: `totalElements` 6,364 / `totalPages` 정상.
  - **B3 회원별 예매 내역**: `GET /admin/accounts/{id}/reservations` — `ReservationService`에 `findReservationsByAccount`(소유자 검증 없는 버전) 추가, A그룹의 `ReservationDetailResponse`(eventName·좌석 포함) 재사용. members 화면에서 행 펼침으로 표시.
  - **B4+B5 콘서트 현황**(`/admin/events`): 신규 `GET /admin/events/stats` — 전 콘서트를 한 번에. `capacity`(좌석 수 + 스탠딩 수용) / `sold`(PAYMENT_CONFIRMED `quantity` 합) / `remaining` / 확정 예약 건수 / 확정 매출. 5개 GROUP BY 쿼리(`SeatRepository.countSeatsPerEvent`, `SectionRepository.sumStandingCapacityPerEvent`, `ReservationRepository.aggregateConfirmedSalesByEvent` + 이벤트 목록)를 메모리 병합. 프론트는 `145/200` 진행바 + `55석 남음`. **api-design.md 6번의 원안(이벤트당 조회 + Redis 실시간 per-section occupancy)과 다르게 구현** — 화면이 목록형이고, "몇 자리 팔렸나"는 매출과 일관되게 확정 기준으로 보는 게 맞아서(문서 6번 수정 완료).
  - **B6 계정 정지(소프트 삭제로 확정, 사용자 확인)**: `AccountStatus`에 `SUSPENDED` 추가, `Account.suspend()`/`reactivate()`, `PATCH /admin/accounts/{id}/suspend`·`/reactivate`. 정지 시 상태 변경 + Redis `refresh_token` 삭제 → 재로그인·재발급 즉시 차단(`AuthService.login`/`refresh`에 `ACCOUNT_SUSPENDED` 체크 추가). **알려진 한계: 이미 발급된 Access Token은 만료(≤30분)까지 유효** — 즉시 차단하려면 JWT 필터가 매 요청 DB 조회해야 해서 성능 트레이드오프, 안 함. 하드 삭제는 매출 통계 왜곡으로 배제. 방어: ADMIN 정지 불가, 이미 정지된 계정 재정지 불가, ACTIVE 재활성화 불가 → 전부 `INVALID_ACCOUNT_STATE`(409, 신규 에러코드). 에러코드 `ACCOUNT_SUSPENDED`도 신규.
  - **검증**: `gradlew test` 17개 통과(B그룹 전용 자동 테스트는 안 씀), 프론트 `tsc -b`/`oxlint`/`vite build` 통과. **admin 토큰(`admin@ticketrush.com`)으로 전 엔드포인트 실측**: 회원목록·필터·pending·events/stats(event 81 → `1/8, 7석 남음, 확정 1건, 10,000원` 실제 예약과 일치)·회원별 예매내역·정지→로그인 차단(`ACCOUNT_SUSPENDED`)→해제→복구·방어 3종(409/409/404) 전부 정상. 프론트 3개 화면 브라우저 렌더는 미확인(사용자 확인 예정).
  - **문서**: `api-design.md` 6번 전면 개정, 에러코드 2개 추가. `progress.md`·`CLAUDE.md`·`decisions.md` 3번·`db-schema.md` `account.status` 갱신.
  - **2026-09-07 아침 재개용 상태**: **B그룹 코드 전부 미커밋**(백엔드 11수정+5신규, 프론트 8수정+4신규, 문서 5개). 커밋 계획은 3개 — `feat(admin):` 백엔드 / `feat(frontend):` 관리자 화면 / `docs:` B그룹 반영. 그 앞 커밋 `86d78ea`(docs 정합화)도 로컬 기준 미푸시로 보임(이미 푸시했으면 무시). **로컬 DB는 완전 초기화 + 재시드됨** — 계정 6개(aaaa@naver.com/`aaaa1234` 관리자, admin@ticketrush.com/`admin1234` 관리자, qqqq·wwwww·eeee `*1234` 일반, seed-organizer@ticketrush.test/`seedorg1234` 주최자), 한국어 이벤트 5개(아이유/성시경/BTS/레미제라블/잔나비, 전부 오픈). 로컬 서버(gradlew bootRun + npm run dev + docker compose)는 켜둔 채 종료.

- **2026-09-07: 프론트 UI 개편 + 라우팅 변경 (로컬 시연/포트폴리오용, 커밋 예정).** 사용자가 "너무 휑하다"며 인터파크/멜론티켓 스타일 스크린샷 5장을 레퍼런스로 줌.
  - **라우팅**: 첫 화면을 로그인 → **공연 목록(`/`)** 으로. `/`·`/events/:id`에서 `ProtectedRoute` 제거(비로그인 열람, `GET /events*`가 이미 permitAll). `EventDetailPage` "예매하기"는 비로그인이면 안내 모달(로그인하러 가기/계속 둘러보기), `LoginPage`가 `location.state.from`으로 원래 공연 상세 복귀.
  - **디자인**: `index.css`에 CSS 토큰 도입(포인트 컬러 `--green: #1ec800` = 인터파크 그린, 사용자 선택). 네비바 검은 배경→흰 배경+그린 로고+회색 링크. 공연 카드/히어로에 색상 그라데이션 **포스터 placeholder**(백엔드에 이미지 데이터 없음 → 공연명 첫 글자 + `poster-p{id%5}` 그라데이션, CSS만). 대기열은 큰 그린 순번(`N번째`)+안내문구. 로그인/회원가입은 가운데 카드 폼. 좌석/예약/관리자 화면은 공유 토큰으로 자동 반영.
  - **관리자 화면은 무채색으로 분리(사용자 요청 "관리자는 굳이 초록 안 입혀도 됨, 깔끔하게만")**: 3개 화면에 `page-admin` 래퍼 추가 → 그 안에서 버튼 차콜(`#33343d`)·진행바 슬레이트(`#5b5f6b`)·링크 파랑(`#2f6fed`). CSS 우선순위 버그 1건 잡음(`.page-admin button:not(.secondary)`가 `.link-button`을 덮어써 "보기" 링크에 검은 배경 박스가 생김 → `:not(.link-button)` 추가).
...
- **웹훅 유실로 붕 뜬 예약 정리(reconciliation) — 알려진 한계로 남김(2026-09-07, 사용자 확인 완료)**: PG(포트원)가 `Transaction.Paid`/`Transaction.Failed` 웹훅을 **끝내 하나도 안 보내는** 경우(포트원 장애, 네트워크 유실, 우리 서버 재시작 중 수신 실패 등), 예약은 `PAYMENT_REQUESTED`에 영구히 남고 `reservation_seat`도 `PAYMENT_REQUESTED`로 남는다. `payment.processing-timeout-millis`(2분) 뒤 `HoldExpiryScheduler`가 Redis 좌석 홀드만 풀어서 — 좌석맵/`SeatStatusRebuildService.rebuild`는 그 좌석을 "빈 자리"로 보여주지만(둘 다 타임아웃 지난 `PAYMENT_REQUESTED`를 점유로 안 침), `ReservationService.requestPaymentInTransaction`의 2차 방어선 `existsBySeatIdAndStatusIn`은 **시간 필터가 없어** 그 좌석을 영구 거절한다 → 아무도 못 사는 "좀비 좌석"(오버셀이 아니라 언더셀, 정합성은 안전). **처리 방침**: 자동 정리 배치는 만들지 않고, 운영자가 포트원 콘솔에서 실제 결제 상태를 확인해 수동으로 예약을 확정/취소한다. 자동 reconciliation 배치는 향후 과제. 로컬에서는 웹훅이 아예 도달 못 해 이 상태가 항상 재현되며(정상), AWS에서는 웹훅이 몇 초 안에 와서 대부분 정상 확정된다.
- **계정 정지 시 기존 예약/결제 처리 — 알려진 한계로 남김(2026-09-07, 사용자 확인 완료)**: `AdminAccountService.suspend`는 `status → SUSPENDED` + Redis refresh token 삭제만 한다(로그인·재발급 차단). 결제까지 끝난 회원을 정지해도 예약(`PAYMENT_CONFIRMED`)·좌석(SOLD)·매출 통계는 그대로 유지되고 환불·좌석 회수는 없다(soft delete를 택한 이유 = 매출 왜곡 방지, B6 항목). 남아있는 Access Token(≤30분, 리허설 설정 4h)으로는 정지 후에도 예약 조회/취소/홀드가 계속 되며(JWT 필터가 요청마다 DB status를 안 봄), 진행 중이던 `PAYMENT_REQUESTED`는 웹훅이 status를 안 보므로 그대로 확정된다(PG에서 이미 결제됐으니 맞는 동작). **처리 방침**: 정지는 인증만 차단하는 것으로 정의하고, 환불·좌석 회수가 필요하면 운영자가 별도 처리한다. reactivate하면 데이터가 그대로라 원상복구된다.
- 스탠딩 예약의 `quantity`가 여러 장일 때 개별 티켓 단위 이력이 필요한지 (지금은 한 예약 행 = N장으로 묶음, db-schema.md 참고)
- **`PORTONE_WEBHOOK_SECRET` 발급 완료(2026-08-27)** — `.env`에 반영됨. 다만 **실제 결제 이벤트로 서명이 맞는지는 아직 확인 전**이다(콘솔 "호출 테스트"는 서명 헤더가 안 옴). 프론트 결제창 SDK 연동은 2026-09-06에 끝났지만 로컬은 포트원 웹훅이 도달하지 못해(퍼블릭 URL 필요) 여전히 검증 불가 — **AWS 재배포 후 실결제/실웹훅으로 서명 형식(Standard Webhooks 가정) 재확인**이 남은 유일한 결제 관련 과제다. 서명 형식이 가정과 다르면 `PaymentWebhookService`를 고치고 재배포 반복 필요(시간 변수).
- **~~프론트 PG SDK 연동(포트원 결제창 호출) 미착수~~ → 로컬까지 완료(2026-09-06, 커밋 `a38a185`/`e633e23`)**. `@portone/browser-sdk` 추가, `SeatHoldPage` 홀드 완료 패널에서 카드(토스페이먼츠 채널)/카카오페이(간편결제 채널) 선택 → `PortOne.requestPayment()` 호출, 리디렉션 복귀 처리 포함. 신규 `GET /api/v1/payments/config`(storeId·채널키 2종), `ReservationResponse`에 `amount`·`orderName` 추가(SDK `totalAmount`/`orderName`으로 전달). 서버 대 서버 재검증(GetPayment)은 여전히 미구현(decisions.md "웹훅 재검증 단순화" 참고) — `PORTONE_API_SECRET`은 `.env`에 있으나 아직 안 씀. 남은 것은 위 항목의 AWS 웹훅 검증뿐.
- **결제창 UI 세부(2026-09-06)**: `payments/config`가 빈 값(=`.env` 미설정)이면 프론트는 결제수단 라디오를 숨기고 예전처럼 "결제 요청"만 한다(결제창 없이). 모바일 리디렉션 방식 복귀 시 URL 쿼리(`paymentId`/`code`)로 결과를 반영한다.
