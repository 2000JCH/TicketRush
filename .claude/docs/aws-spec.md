...
> **2026-09-14 정리**: 이 절은 원래 v1(2026-09-06) 최초 배포 기준으로 작성됐다. 같은 스펙·코드·방법론으로
> 인스턴스를 재생성해 재측정한 **v2(2026-09-10)에서 숫자가 달라졌고(883ms, ~450~500명), 외부 문서
> 공식값은 v2다** — 이건 방법론 오류가 아니라 인스턴스를 재생성할 때마다 생기는 정상적인 변동폭이다
> (`test-results.md` 맨 위 "공식 채택값" 표, 5-2-v2 참고). v1 수치는 지우지 않고 최초 판단 기록으로
> 아래 표에 그대로 남긴다 — 아래 "왜 틀렸는가"의 진단(Redis 자원 경쟁이 원인)은 v1·v2 둘 다에서
> 동일하게 확인돼 유효하다.

| 항목 | 사전 기대 | 실측 v1(09-06) | 실측 v2(09-10, 공식값) | 일치 여부 |
|---|---|---|---|---|
| 동시 300명 P95(홀드→결제) | 로컬(리허설, 캐싱 적용 후) 1,123ms 이상은 유지 | 1,512ms — 통과 | **883ms — 통과** | 대체로 일치 |
| 한계 동시 사용자 | 로컬 리허설(2 vCPU 제한) ~460~480명의 **2배 vCPU니 그 이상** | ~250~300명 — 오히려 더 낮음 | **~450~500명 — 로컬과 비슷한 수준** | v1은 ❌ 불일치, v2는 대체로 일치(그래도 "2배"까지는 아님) |

**v1과 v2가 왜 다른가**: 같은 스펙·코드·방법론인데도 EC2/RDS를 새로 만들 때마다 배정되는 물리
호스트가 달라 생기는 변동폭이다(`test-results.md` 5-2-v2 참고). **원인 진단(Redis가 병목이라는 것)
자체는 v1·v2 둘 다 동일**하게 나왔다 — 달라진 건 "얼마나 버티는가"라는 정도(degree)뿐이다.

**왜 (v1 기준으로) 사전 기대가 틀렸는가**: vCPU를 2배로 늘리면 당연히 한계도 올라갈 거라는 기대는
"무엇이 병목인가"를 CPU 총량으로만 봤기 때문이다. 실제로는 이번에 배포한 코드(캐싱+트랜잭션 범위
축소, 커밋 `603e308`)가 DB 커넥션 병목을 없애면서 **Redis 자원 경쟁**이라는 새 병목을 드러냈고, 이
병목은 vCPU 총량이 아니라 "Redis가 app·Kafka와 한 박스에서 CPU를 나눠 쓰는 구조"(decisions.md 10번,
ElastiCache 미도입) 자체에서 온다 — 로컬 리허설은 이 코드의 진짜 효과를 Redis를 인위적으로 0.5~1
vCPU로 제한해둔 탓에 측정하지 못했었다(`test-results.md` 4-6). AWS가 처음으로 이 코드의 실제 한계를
신뢰성 있게 드러낸 것이다.
...
이 결론은 v1·v2 재측정 모두에서 동일하게 확인됐다(한계 인원 자체는 인스턴스마다 250~300명↔450~500명
사이에서 출렁였다). 상세 근거·요청 종류별 부하 비중·원인 로그는 `test-results.md` 5-2-v2 참고.

---

## E. SLO (Service Level Objective) — 달성 확인 (2026-09-06 v1 / 2026-09-10 v2)

> 로컬 목표 기준선(`test-plan.md` 1번)을 AWS 배포 환경에서 실제로 Gatling 재측정해 검증했다.
> **아래는 공식값(v2, 2026-09-10) 기준이다** — v1(09-06)에서도 판정(통과/실패) 자체는 동일했다.
> 상세 수치·원인 진단은 `test-results.md` 5번(부하/한계), 6번(카오스) 참고.

| 지표 | 목표 | AWS 실측(v2) | 판정 |
|---|---|---|---|
| 오버셀 | 0건(절대) | 0건(부하 + 카오스 2종, 5-2-v2는 N=500 라운드 한정 SQL 확인) | ✅ |
| 동시 300명 P95 홀드→결제 | < 2,000ms | 883ms | ✅ |
| 에러율(경합 제외) | < 1% | 0%(N=300, 300/300 전원 성공) | ✅ |
| Redis 장애 복구 | < 30초 | 18초(재구성 완료 기준) | ✅ |
| Kafka lag 0 도달 | < 60초 | 15초 이내(수동 개입 없이 자동 복구, 로컬보다 개선) | ✅ |

(v1 참고: P95 1,512ms / 에러율 300명 기준 2.3%(계정 토큰 충돌 401, 경합 아님) / Redis 복구 5초 — 판정은 v2와 동일하게 전부 통과)

**종합 판정: SLO 전체 통과.** 다만 v1 측정에서는 목표(300명) 바로 위부터 Redis 자원 경쟁으로 급격히
무너진 적이 있어(D-1 참고), **인스턴스 배정에 따라 실제 여유 폭이 크게 출렁일 수 있다는 것 자체가
핵심 발견**이다 — "지금 목표는 만족하지만 여유 폭은 인스턴스마다 다를 수 있고, 다음 확장 시 손대야
할 지점(Redis 분리)까지 특정했다"가 정직한 결론이다.
